"""Meta-validation experiment for the Gap Continuation study.

Consumes the assembled trial matrix (registered as a dataset by the
...
from quant_research — the canonical math engine:
    ...
    - deflated_sharpe_ratio: Bailey & Lopez de Prado (2014). Is the
      best trial Sharpe still significantly positive once the number of
      searched trials is accounted for? Per-observation (daily) Sharpes,
      n_obs = number of finite days.
    - reality_check_p_value: White (2000). Block-bootstrap p-value for
      "even the best of these trials has no edge", computed on the
...
No statistics are computed outside quant_research in this module; the
trial matrix itself is assembled from registry artifacts by the
...
def run(ctx):
    ...
    def compare(name, other, other_label):
        ...
        tests.append({
            ...
            "statistic": float(t),
            "p_value": float(p),
            "conclusion": ("best trial mean %.5f vs %s mean %.5f: p=%.4f "
