"""Overnight Gap Fading strategy experiment (Campaign C002, H-MS-01).
...
Hypothesis under test
    ...
    close of day t+hold_days-1) mean-reverts: gap up -> short,
    gap down -> long (the exact sign flip of C001's continuation rule;
    identical construction otherwise).

Design
    - Entries: day t qualifies if the signed gap magnitude exceeds the
      threshold (direction filter: "up" = gap >= +thr, "down" = gap <=
      -thr, "both" = |gap| >= thr).
    - Fade position: up gap -> short, down gap -> long (inverse of the
      C001 continuation convention).
...
every test below is computed by it, seeded with ctx.seed):
    - permutation_test_signal: one-sided p-value that entry-day
      ...
      hold-day returns under the same directional fade construction
      (null = entries are random days; the fade construction flips the
      C001 signed pool).
...
def _build_trades(ohlc, gap, hold: int, threshold: float, direction: str,
                  cost_bps: float):
    """Non-overlapping fade trades. Returns (trade_rets, pnl_daily, entries)."""
    n = ohlc.shape[0]
    open_ = ohlc[:, 0]
    close = ohlc[:, 3]
...
def run(ctx):
    ...
    # Nullity pool: the fade directional rule (short after up-gaps, long
    # after down-gaps) applied on ANY day — the construction-matched
    # signed pool is the negation of C001's continuation pool. The null
    # is "entries are random days under the strategy's own construction".
    ...
    if n_trades < 3:
        ...
        # --- Nullity gate: entry days vs the pool of all possible fade
        # trades (same directional construction on any day) ---
